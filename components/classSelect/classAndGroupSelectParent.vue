...
<script>
	...
	export default {
		...
		methods: {
			getClassList() {  //当前班主任所在的班级
				Request.get(Request.APIPAR.selectClassGroupListByUserId, {
					...
					if (res.data.code == 200) {
						...
						if (res.data.rows.length > 0) {
							if (this.classId=='') {
								...
								this.$emit('classIdShow', this.classList[0]);
							}
						}
						this.listTotal = res.data.total
						this.isLoad = true
						console.info('班级classList', this.classList)
					} else {
						uni.showToast({
							title: '班级加载失败',
							icon: 'none',
