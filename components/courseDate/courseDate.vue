<template>
	<view class="">
		...
		<view class="myDateShow flex justify-between align-center text-white" >
			<view class="" @click="changeWeek('prev')" v-if="prevActive"><text class="cuIcon-backwardfill"></text>上周</view>
			<view class="text-lg text-bold" ref="t1">{{ nowSelectDateString }}</view>
			<view class="" @click="changeWeek('next')" v-if="prevActive">下周<text class="cuIcon-play_forward_fill"></text></view>
		</view>
		<view class="" style="display: flex;height:163rpx ;">
			<view class=" padding-top-sm text-lg" style="width: 10%;text-align: center;color: #2E9C9B;font-weight: bold;">
				<view class=""  style="font-size: 32rpx;">{{myMo}}</view>
				<view class="" style="padding-top: 24rpx;" >月</view>
			</view>
			<view class="cal-content " style="width: 90%;">
		...
		methods: {
			initData() {
				...
				this.$set(this.baseData, 'selectedDate', {
					...
					timeSpan: +lastSat,
					m: new Date(lastSat),
					yy: lastSat.getFullYear(),
					mm: lastSat.getMonth() + 1,
					ymd: `${lastSat.getFullYear()}-${lastSat.getMonth() + 1}-${lastSat.getDate()}`
				});
				this.myMo = lastSat.getMonth() + 1
				// 寻找到距离当前日期最近的周六
				...
				this.current = 0;
				this.$emit('myWeek', this.weeks[this.current]);
				// console.info("本周1",this.weeks[this.current])
			},
			swiperFisnish(e) {
				let current = e.detail.current;
				this.changeWeeks(current > this.baseData.current, current);
				this.$set(this.baseData, 'upCurrIndex', current);
				this.myMo = this.weeks[current][0].m.getMonth() + 1
				// console.info("本周",this.weeks[current])
				this.$emit('myWeek', this.weeks[current]);
			},
