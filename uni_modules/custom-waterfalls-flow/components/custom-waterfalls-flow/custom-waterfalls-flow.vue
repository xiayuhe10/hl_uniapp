	...
	export default {
		...
		methods: {
			...
			loadImages(idx = 0) {
				...
				// console.info("预加载图片",this.data.list)
			...
			// 刷新
			refresh() {
				// console.info("刷新",this.isLoaded,this.value)
				if (!this.isLoaded) {
			...
			change(newValue) {
				...
				console.info("changelist",this.data.list)
			...
			imgLoad(item, c) {
				...
				// console.info("图片加载完成",this.data[`column_${c}_values`])
				this.initValue(i + 1);
			},
			// 图片加载失败
			imgError(item, c) {
			...
			loaded() {
				...
				// console.info("渲染结束",this.data.list)
				// console.info("渲染结束",this.refreshDatas)
			},
			// 单项点击事件
			wapperClick(item) {
		...
		watch: {
			value: {
				...
				handler(newValue, oldValue) {
					setTimeout(() => {
						this.$nextTick(() => {
							// console.info("isRefresh",this.isRefresh)
							if (this.isRefresh) return false;
							console.info("isLoaded",this.isLoaded)
							if (this.isLoaded) {
								console.info("watchnewValue",newValue)
								...
								// console.info("addsnewValue",newValue)
								// console.info("adds",this.adds)
							}
						})
					}, 10)
				}
