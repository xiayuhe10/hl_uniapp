		...
		methods: {
			...
			show() {
				...
				that.getDomStyles(styles => {
					if (styles) {
						that.lsjFile.dom.setStyle(styles)
						console.info(styles)
					} else {
						let push = {
							"top": "527px",
						...
						that.lsjFile.dom.setStyle(push)
						console.info(push)
					}
			...
			onClick() {
				...
				console.info("点击选择附件只允许上传", this.wxFileType, count)
