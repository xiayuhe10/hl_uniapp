export class LsjFile {
	...
	create(path) {
		if (!this.dom) {
			...
			// #ifdef APP-PLUS
				...
				// this.dom.addEventListener('loaded', () => {
	...
	addFile(file) {
		let name = file.name;
		this.debug&&console.log(file,'文件名称',name,'大小',file.size);
		if (file) {
			// 限制文件格式
	...
	_changeFilesItem(item) {
		...
		// this.debug&&console.log('onprogress_changeFilesItem',JSON.stringify(item));
	}
	
	_uploadHandle(item) {
		// this.debug&&console.log('_uploadHandle_uploadHandle',JSON.stringify(item));
		item.type = 'loading';
