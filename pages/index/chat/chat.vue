	...
	<view class="content avatarwrap scrollViewContent" id="scrollViewContent" @click="show = false"
		...
		<scroll-view class="scrollview chat scroll-box" id="scrollview" scroll-with-animation scroll-y="true"
			...
			<view class="chat-main" :style="{paddingBottom:inputh+'px'}">
				<view class="cu-chat" v-for="(item,index) in msgList" :key="index"  :index="index" :id="'scroll-'+ index">
					<!--我的聊天 文字 start-->
					...
					<!-- 右边消息 -->
					<view class="cu-item self  " v-if="item.fromId==fromId && item.msgType!='RECALL_MSG'">
						<view class="main">
							<!--文字start-->
							...
							<view class="plane-once content " style="width:200px;"
								@tap="onOpenDoc(fileUrl+item.content.url)" @longpress="longpress($event,item,1)"
								...
								<view class="margin-left-xs">
									...
									<view style="color: gray;font-size: 2rpx;">{{changeKM(item.content.size)}}</view>
								</view>
							...
							<image :src="fileUrl+item.content" v-if="item.contentType=='IMAGE'&&item.msgType=='MSG'"
								@longpress="longpress($event,item,1)" @touchend="touchend" class="radius"
								mode="widthFix" @tap="previewImg(item.content,index)"></image>
							<!-- 视频 -->
							<video :id="'video'+index" :src="fileUrl+item.content" v-if="item.contentType=='VIDEO'&&item.msgType=='MSG'"
								...
								controls :poster="fileUrl+item.content+'?x-oss-process=video/snapshot,t_0,f_jpg'"></video>
							<!-- 语音 -->
					...
					<view class="cu-info round text-center" v-if="item.fromId==fromId && item.msgType=='RECALL_MSG'">
						我撤回一条消息!</view>
					<!-- 左边消息 -->
					<view class="cu-item left" v-if="item.fromId!=fromId&&item.msgType!='RECALL_MSG'">
						<!-- 头像 -->
						<view class="cu-avatar radius bg-white"
							:style="{backgroundImage:`url(${fileUrl+item.fromAvatar})`}"
							@click="patriarchInfo(item.fromId)">
						...
						<view class="main">
							...
							<view class="content shadow plane-once" :class="item.chatType=='GROUP'?'sss':''"
								v-if="item.contentType=='TEXT'&&item.msgType=='MSG'"
								@longpress="longpress($event,item,2)" @click="click()" @touchend="touchend">
								<text>{{item.content}}</text>
							</view>
							<!--文件start-->
							<view class="plane-once content " :class="item.chatType=='GROUP'? 'margin-top':'' "
								...
								@longpress="longpress($event,item,2)"
								v-if="item.contentType=='FILE'&&item.msgType=='MSG'">
								<view style="height: 100%;width: 20%;">
									<image style="display: block;width: 80rpx;height: 70rpx;
									margin-top:11rpx;" :src="eventType[item.content.type].src"></image>
							...
							<!-- 照片 -->
							<image :src="fileUrl+item.content" v-if="item.contentType=='IMAGE'&&item.msgType=='MSG'"
								@longpress="longpress($event,item,2)" @touchend="touchend" class="radius imgSizeLeft"
								mode="widthFix" @tap="previewImg(item.content,index)">
							</image>
							<!-- 视频 -->
							<video :id="'video'+index" :src="fileUrl+item.content" v-if="item.contentType=='VIDEO'&&item.msgType=='MSG'"
								class="radius" show-mute-btn="true" x-webkit-airplay="true" webkit-playsinline="true"
							...
							<view class="message content bg-white  plane-once  align-center"
								:class="item.chatType=='GROUP'? 'margin-top':'' " @longpress="longpress($event,item,2)"
								@click="click(item)" @touchend="touchend"
								v-if="item.contentType=='VOICE'&&item.msgType=='MSG'" @tap="playVoice(item)">
								...
								<view class="content2" v-if="item.id != voiceShowId ">
									...
									<view class="common2 middle_0xs"></view>
									<view class="common2 min2"></view>
								</view>
		...
		<view style="z-index: 9999;">
			<submit @inputs="inputs" @heights="heights" :flag="myFlag"></submit>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
				...
				Request.get(Request.API.msgList, {
					...
					if (res.data.code == 200) {
						...
						for (var i = 0; i < resData.length; i++) {
							if (resData[i].contentType == 'VOICE' || resData[i].contentType == 'FILE') {
								// console.info(resData[i])
			...
			//跳到家长详情
			patriarchInfo(userId) {
				// console.info(userId)
				uni.navigateTo({
					url: "../../contact/patriarchInfo?userId=" + userId + "&groupId=null"
				})
			},
			longpress(e, item, index) { // 长摁时间
				this.chehuiShow = index
				// console.info(this.chehuiShow, item.id)
			...
			popChoice(item) { // 长摁显示窗口的点击事件
				...
				if (item == 1) {
					...
					Request.delete(Request.API.delectImChatMsgById + this.popChoiceText.id).then(res => {
						console.log("删除：", res)
						this.getList();
					})
				} else if (item == 3) {
					...
					this.show = false
					Request.post(Request.API.recallImChatMsgById, {
						"id": this.popChoiceText.id
					}).then(res => {
						console.log("撤回：", res)
						this.getList();
					})
